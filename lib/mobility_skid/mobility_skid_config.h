...
static const float TICKS_TO_RAD = 2 * M_PI / COUNTS_PER_REV;
#elif
static const float PULSES_PER_REV = ENCODER_PPR * MOTOR_REDUCTION; // encoder ticks per wheel rev - 312
static const float TICKS_TO_RAD = 2 * M_PI / PULSES_PER_REV;
#endif
...
static const float MAX_WHEEL_ANGULAR = MAX_RPM_ALLOWED * 2 * M_PI / 60; // rad/s
...
static const float PID_LEFT_KC = 1.0 * PID_LEFT_KF; // PID gain
static const float PID_LEFT_TI = 1.0;               // I Time
static const float PID_LEFT_TD = 0.0;               // D Time

static const float PID_RIGHT_KF = 126 / MAX_SPEED;    // feed forward control
static const float PID_RIGHT_KC = 1.0 * PID_RIGHT_KF; // PID gain
static const float PID_RIGHT_TI = 1.0;                // I Time
static const float PID_RIGHT_TD = 0.0;                // D Time

// Wiring pins
#define SABERTOOTH_SERIAL UART_NUM_2
...
// Task periods
#define PERIOD_CONTROL_MS 50 // 20Hz PID control
#define PERIOD_REPORT_MS 200 // 5Hz report joint states
